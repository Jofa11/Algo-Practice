...
const tesTown = 'London';
const testStrng = '50,100';
...
function mean(town, strng) {
	...
	const nums = newArr.map(Number);

	return nums.reduce((a, b) => a + b) / nums.length;
}

// console.log(mean(tesTown, testStrng));
...
function variance(town, strng) {
	...
	return Math.abs(nums.reduce((a, b) => a - b));
}

// console.log(variance(tesTown, testStrng));
...
const goodData = data.split('\n');

const betterData = goodData.split(',');
console.log(betterData);

// Didn't solve this one. Top solutions:
...
// #2
const annualRecords = (town, s) =>
	s
		.match(new RegExp(`${town}:(.*)(\n|$)`, ''))[1]
		.match(/\d+\.\d/g)
		.map(Number);
const mean = (...args) => {
	try {
...
const variance = (...args) => {
	try {
		return (
			annualRecords(...args).reduce(
				(res, num) => res + (num - mean(...args)) ** 2,
				0
			) / 12
		);
	} catch (err) {
